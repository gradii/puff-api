...
import exploreSourceMap from 'source-map-explorer';
...
export function generateStats(
  ...
  outputAssets.forEach((asset: string) => {
    ...
    if (asset.endsWith('.js')) {
      ...
      try {
        exploreSourceMap(join(outputPath, asset)).then((sourceMapData) => {
          sourceMapData.bundles.forEach(
            bundleResult => {
              Object.keys(bundleResult.files).forEach(_file => {

                const size = bundleResult.files[_file].size;
