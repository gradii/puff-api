...
/**
 * Script that builds the puff-app as a static web package that will be
...
/** Output path for the Bazel puff-app web package target. */
const webPackagePath = join(bazelBinPath, 'src/puff-app/web_package');

/** Destination path where the web package should be copied to. */
const distPath = join(projectDirPath, 'dist/puff-app-web-pkg');

// Build web package output.
exec('yarn -s bazel build //src/puff-app:web_package --config snapshot');
...
async function build() {
  ...
  const fileMap = [
    {
      key: 'puff-app-web-pkg',
      cwd: './dist/puff-app-web-pkg',
      source: ['./'],
